...
Inputs:
    ...
    --dataset-tag     One of {ixi, fastmri, abide}. Recorded in every manifest row
...
def main(
    ...
    device: str = typer.Option(
        ...
        "--device",
        help=(
            "Torch device for FFT-based corruptions. 'cpu' (default), 'cuda', "
            "or 'auto' (selects 'cuda' when available, else 'cpu'). On A100 "
            "GPUs the FFT corruptions (motion / ghosting / spike) run "
            "~100-300x faster than CPU; transforms that don't support GPU "
            "(e.g. bias_field) auto-fall-back to CPU per-scan."
        ),
    ),
) -> None:
