...
# BBillot/SynthSeg ships a bundled synthseg_1.0.h5 at ${SS_MODELS}/, but --parc
# / --robust / --qc need the full set of .h5 + label .npy files at ${FS_HOME}/
# models. The previous "skip if non-empty" guard left the bundled-only state
# in place and made --parc fail with `AssertionError: model path does not
# exist.`. The bundled .h5 is also at ${FS_HOME}/models so removal loses
# nothing.
